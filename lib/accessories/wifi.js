    ...
    if (this.fallback) {
        ...
        tr064.initTR064Device("fritz.box", 49000, function(err, device) {
            ...
            device.startEncryptedCommunication(function(err, sslDev) {
                ...
                sslDev.login(this.platform.config.username, this.platform.config.password);

                // remember device
                this.tr64device = device;

                // remember service
                let wifiService = "urn:dslforum-org:service:WLANConfiguration";
                this.tr64service = device.services[wifiService + ":3"] || device.services[wifiService + ":2"];
...
FritzWifiAccessory.prototype.getOn = function(callback) {
    ...
    this.tr64service.actions.GetInfo(function(err, res) {
        if (err) {
            this.platform.log.error(err);
            return;
        }
...
FritzWifiAccessory.prototype.setOn = function(on, callback, context) {
    ...
    this.tr64service.actions.SetEnable(payload, function(err, res){
        if (err) {
            this.platform.log.error(err);
            return;
        }
